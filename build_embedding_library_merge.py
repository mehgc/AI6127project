...
# ====== ✅ 统一路径配置 ======
SAVE_DIR = "./outputs"  # 所有保存的文件都放在这个目录下
...
def load_and_filter_datasets(min_q_len=500, min_opt_len=30, verbose=True):
    """
    加载并筛选 MedQA / MedMCQA / PubMedQA 三个数据集
    返回筛选后统一格式的数据 + 每个数据集的统计量
    """
    print("📥 正在加载数据集...")
    medqa = load_dataset("bigbio/med_qa", "med_qa_en_4options_source", trust_remote_code=True)['train']
    medmcqa = load_dataset("medmcqa")['train']
    ...
    for ex in medqa:
        if len(ex['question']) > min_q_len and average_option_length(ex['options']) > min_opt_len and is_case_based(ex['question']):
            all_data.append(format_medqa(ex))
            stats["medqa"] += 1

    for ex in medmcqa:
        if len(ex['question']) > min_q_len and average_option_length([
            {"key": "A", "value": ex['opa']}, {"key": "B", "value": ex['opb']},
            {"key": "C", "value": ex['opc']}, {"key": "D", "value": ex['opd']}
        ]) > min_opt_len and is_case_based(ex['question']):
            all_data.append(format_medmcqa(ex))
            stats["medmcqa"] += 1

    for ex in pubmedqa:
        if ex['final_decision'] in ['yes', 'no', 'maybe']:
            all_data.append(format_pubmedqa(ex))
            stats["pubmedqa"] += 1
